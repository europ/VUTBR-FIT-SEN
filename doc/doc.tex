...
We have done fifty measurements on different persons with our device and valid device. These values are shown in Table \ref{fig:measurements}. Column `\textit{Person}' represents each person, column `\textit{Value 1}' includes values measured via devices shown in Section \ref{sec:intro} and column `\textit{Value 2}' includes values measured via valid device TODO.

\section{Conclusion}
...
Vliv okamžitého psychikého stavu na měření (strach a stres z měření).
